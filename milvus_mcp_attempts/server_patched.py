...
class MilvusConnector:
    ...
    async def search_collection(
        ...
        try:
            # Instead of using "sparse", use a field that exists in your collection
            search_params = {"params": {"drop_ratio_search": drop_ratio}}

            results = self.client.search(
                collection_name=collection_name,
                data=[query_text],
                anns_field="vector",  # Change "sparse" to "vector" 
                limit=limit,
    ...
    async def vector_search(
        ...
        limit: int = 5,
        output_fields: Optional[list[str]] = None,
        metric_type: str = "IP",  # Changed default to IP
    ) -> list[dict]:
        """Perform vector similarity search on a collection."""
        try:
            # Always use a supported metric type
            actual_metric = "IP" if metric_type.upper() not in ["IP", "L2"] else metric_type.upper()
    ...
    async def create_collection(self, collection_name: str, schema: dict[str, Any], index_params: Optional[dict[str, Any]] = None) -> bool:
        try:
    ...
    async def create_index(
        ...
        try:
            ...
            # Ensure metric_type is supported
            actual_metric = "IP" if metric_type.upper() not in ["IP", "L2"] else metric_type.upper()
...
mcp = FastMCP("Milvus", lifespan=server_lifespan, host="0.0.0.0", port=8000)
...
@mcp.tool()
async def milvus_create_collection(collection_name: str, collection_schema: dict[str, Any], index_params: Optional[str] = None, ctx: Context = None) -> str:
    connector = ctx.request_context.lifespan_context.connector
    ...
    for field in collection_schema.get("fields", []):
        if field.get("type") == "FLOAT_VECTOR":
            ...
            dimension = field.get("dim", 1536)
        elif field.get("is_primary", False):
            primary_field = field.get("name", "id")
            id_type = DataType.INT64 if field.get("type") == "INT64" else DataType.VARCHAR
    ...
    import json
    try:
        idx_params = json.loads(index_params) if index_params else None
        if idx_params and "metric_type" in idx_params:
            idx_params["metric_type"] = "L2"
    except Exception as e:
        return f"Failed to parse index_params: {str(e)}"

    success = await connector.create_collection(collection_name=collection_name, schema=schema, index_params=idx_params)

    try:
        await connector.load_collection(collection_name=collection_name)
        return f"Collection '{collection_name}' created, indexed with L2 metric, and loaded into memory."
    except Exception as e:
        return f"Collection created but failed to load: {str(e)}"
...
async def on_startup():
    print("⚙️ Initializing Milvus client...")
    mcp.milvus = Milvus(
        uri=mcp.config["milvus_uri"],
        token=mcp.config.get("milvus_token"),
        db_name=mcp.config.get("db_name"),
    )
    print("✅ Milvus client initialized.")

if __name__ == "__main__":
    print("🐍 Launching Milvus MCP server...")
    ...
    try:
        ...
        print(f"  MILVUS_URI = {os.environ.get('MILVUS_URI')}")
        print(f"  MILVUS_TOKEN = {os.environ.get('MILVUS_TOKEN')}")
        print(f"  MILVUS_DB = {os.environ.get('MILVUS_DB')}")
    ...
    mcp.on_startup = on_startup

    print("🚀 Starting FastMCP server...")
    
    # ✅ Define FastAPI app manually
app = FastAPI()

# ✅ Now create FastMCP instance using the app
mcp = FastMCP(
    tools=[
        create_collection,
        insert_data,
...
if __name__ == "__main__":
    import uvicorn
    print("🚀 Starting FastMCP server via uvicorn...")
    uvicorn.run("server_patched:app", host="0.0.0.0", port=8000, reload=True)
